...
class Defender {
  constructor(x, y) {
    ...
    this.projectiles = [];
    this.timer = 0;
    this.defenderType = defenderTypes[chosenDefender - 1];
    this.frameX = 0;
    this.frameY = 0;
    this.minFrame = 0;
    this.maxFrame = 16;
...
canvas.addEventListener('click', function () {
  ...
  const indexOf = defenders.findIndex(
    (d) => d.x === gridPositionX && d.y === gridPositionY
  );
  if (indexOf > -1) return;
...
function chooseDefender() {
  ...
  if (collision(card1, mouse) && mouse.click) {
    chosenDefender = 1;
  }
  if (collision(card2, mouse) && mouse.click) {
    chosenDefender = 2;
  }
  if (chosenDefender === 1) {
    card1stroke = 'gold';
